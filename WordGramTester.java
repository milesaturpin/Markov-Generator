...
public class WordGramTester {
	...
	public void testToString(){
		...
		WordGram c = new WordGram(words,1,3);
		WordGram d = new WordGram(words,2,2);
		assertEquals("toString fail on a", a.toString(), "{\"apple\",\"zebra\",\"mongoose\",\"hat\"}");
		assertEquals("toString fail on b", b.toString(), "{\"apple\",\"zebra\",\"mongoose\"}");
		assertEquals("toString fail on c", c.toString(), "{\"zebra\",\"mongoose\",\"hat\"}");
		assertEquals("toString fail on d", d.toString(), "{\"mongoose\",\"hat\"}");
	}
	...
	public void testShiftAdd(){
		...
		WordGram c = new WordGram(words,1,3);
		WordGram d = new WordGram(words,2,2);
		assertEquals("shiftAdd fail on a", a.shiftAdd("cat").toString(), "{\"zebra\",\"mongoose\",\"hat\",\"cat\"}");
		assertEquals("shiftAdd fail on b", b.shiftAdd("cat").toString(), "{\"zebra\",\"mongoose\",\"cat\"}");
		assertEquals("shiftAdd fail on c", c.shiftAdd("cat").toString(), "{\"mongoose\",\"hat\",\"cat\"}");
		assertEquals("shiftAdd fail on d", d.shiftAdd("cat").toString(), "{\"hat\",\"cat\"}");
	}
